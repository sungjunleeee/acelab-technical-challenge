...
| `tests/` | Validation set (`validation_set.py`, 10 `BriefCase`s spanning content + input-quality axes), per-stage unit tests with hand-written mocks (`test_stages.py`, runs in <1s on every save), and live-API e2e tests behind the `pytest -m e2e` marker — including a hallucination audit that fails when `why_it_fits` makes un-corroborated cert claims. |
